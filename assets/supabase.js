...
supabase.auth.onAuthStateChange(async (event, session) => {
  if(event === 'SIGNED_IN' && session?.user){
    ...
    if(!data){ await supabase.from('profiles').insert([{id: session.user.id, email: session.user.email, name: session.user.user_metadata.full_name || session.user.email, lang: CONFIG.default_lang}]) }
    if(window.location.pathname.includes('login.html')) window.location = 'index.html';
  }
})
