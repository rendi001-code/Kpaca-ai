...
app.post('/api/chat', unggah.single('gambar'), async (req, res) => {
  ...
  let konten = [{ type: "text", text: teks || "Apa isi gambar ini?" }];
  ...
  try {
    const r = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "Kamu KPACA AI, jawab bahasa Indonesia santai dan jelas." },
  ...
  } catch (e) { res.json({ jawaban: "Kesalahan: " + e.message }); }
});
